...
1. **Warm Start**: During `main.py` startup, an asynchronous task spawns to pre-load `BAAI/bge-large-en-v1.5` weights and execute a dummy warmup batch through PyTorch. This ensures subsequent live requests return matching metrics under **40ms**.
2. **Device Selection ([gpu_init.py](file:///d:/Projects/candid/backend/gpu_init.py))**: PyTorch queries the hardware layer using `torch.cuda.is_available()`. This unified API detects both NVIDIA CUDA cores and AMD ROCm Instinct GPU targets.
...
- The application automatically handles table generation and migrations during startup:
  ...
  ```
- No separate database setup commands are necessary. Tables are mapped using SQLAlchemy ORM inside [models.py](file:///d:/Projects/candid/backend/models.py).
